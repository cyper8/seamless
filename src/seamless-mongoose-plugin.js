module.exports = exports = function SeamlessMongoosePlugin (schema){
  var schema = schema;
  console.log("Seamless Plugin for Mongoose");
  var buffer={},ts={},cl={};
  ...
  schema.statics.seamlessDataLink = function(req,res,next){
    ...
    function Respond(docs){
      console.log("got data, returning to peer");
      var ds;
    ...
    if (buffer[id] && (req.method === "GET")) {
      ...
      if (req.method === "GET") {
        ...
        if (req.body instanceof Array) {
          ...
          if (req.body._id) {
            return this.findByIdAndUpdate(req.body._id,req.body,{new:true})
            .then(Respond).catch(Fail);
          }
          return this.findOneAndUpdate(req.params,req.body,{new:true})
          .then(Respond).catch(Fail);
        }
      }
    }
